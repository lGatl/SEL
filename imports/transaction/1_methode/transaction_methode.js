...
Meteor.methods({
	creeTransaction:(annonce_id, proposition_id)=>{ 
		...
		if((annonce._id==proposition.annonce_id)&&
			...
			BD.Users.update({_id:prestataire._id},{$set:{"profile.solde":(prestataire.profile.solde + proposition.prix)}});
			...
			BD.Transaction.insert({
				...
				prestataire: {...prestataire, profile:{...prestataire.profile,solde:(prestataire.profile.solde + proposition.prix)}},
